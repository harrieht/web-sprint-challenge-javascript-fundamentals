...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
//  The reason why the nested function can access the variable internal is because its within the scope.
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(zooAnimals) {
   const newLowerCase = zooAnimals.map(animal => {
...
console.log(lowerCaseNames(zooAnimals));
  ...
  */
  ...
  function lowPopulationAnimals(array){
   const newLowPop = array.filter(function(item) {
...
*/
const cube = new CuboidMaker({length: 4, width: 5, height: 5});
CuboidMaker.prototype.volume = function(){
  const multiply = this.length * this.width *this.height;
  return multiply;
}
